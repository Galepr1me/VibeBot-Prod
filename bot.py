...
async def pack_slash(interaction: discord.Interaction):
    ...
    # Debug: Add pack_cards length info
    embed.add_field(
        name="🔍 Debug Info",
        value=f"Generated {len(pack_cards)} cards",
        inline=False
    )
...
# Debug Commands
@bot.tree.command(name='debug_cards', description='Debug card system (Admin only)')
@app_commands.default_permissions(administrator=True)
async def debug_cards_slash(interaction: discord.Interaction):
    """Debug the card system to identify issues"""
    global _current_db_type
    
    debug_info = []
    
    try:
        # First, check basic card library
        library_count = len(card_game.card_library)
        debug_info.append(f"📚 Cards in library: {library_count}")
        
        # Try to connect to database
        conn = get_db_connection()
        c = conn.cursor()
        debug_info.append(f"✅ Database connection successful ({_current_db_type})")
        
        # Check if cards table exists and has data
        try:
            c.execute('SELECT COUNT(*) FROM cards')
            card_count = c.fetchone()[0]
            debug_info.append(f"📊 Cards in database: {card_count}")
        except Exception as e:
            debug_info.append(f"❌ Error counting cards: {str(e)}")
            conn.close()
            raise e
        
        # Test getting a specific card
        test_card_name = "Fire Sprite"  # First card in the library
        try:
            card_data = get_card_by_name(test_card_name)
            if card_data:
                debug_info.append(f"✅ Found '{test_card_name}' in database: ID {card_data[0]}")
            else:
                debug_info.append(f"❌ Could not find '{test_card_name}' in database")
        except Exception as e:
            debug_info.append(f"❌ Error looking up card: {str(e)}")
        
        # Check user's current collection
        try:
            user_collection = get_user_collection(interaction.user.id)
            debug_info.append(f"🎴 User's collection size: {len(user_collection)} unique cards")
            
            # Show first few cards in collection if any
            if user_collection:
                debug_info.append("📋 User's cards:")
                for i, card in enumerate(user_collection[:3]):
                    debug_info.append(f"   • {card[1]} (Qty: {card[9]})")  # name and quantity
        except Exception as e:
            debug_info.append(f"❌ Error getting user collection: {str(e)}")
        
        # Show some sample cards from database
        try:
            c.execute('SELECT name, card_id FROM cards LIMIT 5')
            sample_cards = c.fetchall()
            if sample_cards:
                debug_info.append("📋 Sample cards in database:")
                for name, card_id in sample_cards:
                    debug_info.append(f"   • {name} (ID: {card_id})")
            else:
                debug_info.append("❌ No sample cards found in database")
        except Exception as e:
            debug_info.append(f"❌ Error getting sample cards: {str(e)}")
        
        # Test pack token system
        try:
            user_tokens = get_user_pack_tokens(interaction.user.id)
            debug_info.append(f"🎫 User's pack tokens: {user_tokens}")
        except Exception as e:
            debug_info.append(f"❌ Error getting pack tokens: {str(e)}")
        
        conn.close()
        
        embed = discord.Embed(
            title="🔍 Card System Debug Report",
            description="\n".join(debug_info),
            color=0x00ff00
        )
        ...
        error_msg = f"Error during debug: {str(e)}\nType: {type(e).__name__}"
        ...
        # Add any debug info we collected before the error
        if debug_info:
            embed.add_field(
                name="Debug Info Before Error",
                value="\n".join(debug_info),
                inline=False
            )
        ...
        try:
            ...
            # If even the error response fails, try a simple message
...
@app_commands.default_permissions(administrator=True)
@app_commands.describe(
    user='User to give tokens to',
    quantity='Number of tokens to give (default: 5)',
    pack_type='Type of pack tokens (default: standard)'
)
async def give_tokens_slash(interaction: discord.Interaction, user: discord.Member, quantity: int = 5, pack_type: str = 'standard'):
    """Give pack tokens to a user - Admin only"""
    
    try:
        success = add_pack_tokens(user.id, pack_type, quantity)
        
        if success:
            # Get user's total tokens after adding
            user_tokens = get_user_pack_tokens(user.id)
            total_tokens = user_tokens.get(pack_type, 0)
            
            embed = discord.Embed(
                title="✅ Pack Tokens Given",
                description=f"Successfully gave **{quantity}** {pack_type} pack tokens to {user.mention}",
                color=0x00ff00
            )
            
            embed.add_field(
                name="📊 Token Status",
                value=f"🎫 **{user.display_name}** now has **{total_tokens}** {pack_type} tokens",
                inline=False
            )
            ...
            embed = discord.Embed(
                title="❌ Failed to Give Tokens",
                description="There was an error giving pack tokens to the user.",
                color=0xff0000
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
...
@bot.tree.command(name='give_cards', description='Give specific cards to a user (Admin only)')
@app_commands.default_permissions(administrator=True)
@app_commands.describe(
    user='User to give cards to',
    card_name='Name of the card to give',
    quantity='Number of cards to give (default: 1)'
)
async def give_cards_slash(interaction: discord.Interaction, user: discord.Member, card_name: str, quantity: int = 1):
    """Give specific cards to a user - Admin only"""
    ...
    try:
        ...
        if not card_data:
            # Show available cards
            available_cards = [c['name'] for c in card_game.card_library[:10]]  # First 10 cards
            embed = discord.Embed(
                title="❌ Card Not Found",
                description=f"Could not find card: **{card_name}**",
                color=0xff0000
            ...
            embed.add_field(
                name="Available Cards (examples)",
                value="\n".join(f"• {name}" for name in available_cards),
                inline=False
            )
            embed.set_footer(text="Use exact card names • Case sensitive")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        
        # Add the card to user's collection
        card_id = card_data[0]
        success = add_card_to_collection(user.id, card_id, quantity)
        
        if success:
            embed = discord.Embed(
                title="✅ Cards Given",
                description=f"Successfully gave **{quantity}x {card_name}** to {user.mention}",
                color=0x00ff00
            )
            
            # Show card details
            card_info = next((c for c in card_game.card_library if c['name'] == card_name), None)
            if card_info:
                element_info = card_game.elements[card_info['element']]
                embed.add_field(
                    name="🃏 Card Details",
                    value=f"{element_info['emoji']} {card_info['element'].title()} • {card_info['rarity'].title()}\n"
                          f"⚔️ {card_info['attack']} ATK • ❤️ {card_info['health']} HP • 💎 {card_info['cost']} Cost",
                    inline=False
                )
            ...
            embed = discord.Embed(
                title="❌ Failed to Give Cards",
                description="There was an error adding the cards to the user's collection.",
                color=0xff0000
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
...
@bot.tree.command(name='check_tokens', description='Check a user\'s pack tokens (Admin only)')
@app_commands.default_permissions(administrator=True)
@app_commands.describe(user='User to check tokens for')
async def check_tokens_slash(interaction: discord.Interaction, user: discord.Member):
    """Check a user's pack tokens - Admin only"""
    
    try:
        user_tokens = get_user_pack_tokens(user.id)
        
        embed = discord.Embed(
            title=f"🎫 {user.display_name}'s Pack Tokens",
            color=0x3498db
        )
        
        if user_tokens:
            token_text = ""
            for pack_type, quantity in user_tokens.items():
                token_text += f"🎫 **{pack_type.title()}**: {quantity} tokens\n"
            embed.add_field(name="Token Inventory", value=token_text, inline=False)
        else:
            embed.description = "This user has no pack tokens."
        
        # Also show their collection stats
        collection = get_user_collection(user.id)
        total_cards = sum(card[9] for card in collection) if collection else 0
        unique_cards = len(collection) if collection else 0
